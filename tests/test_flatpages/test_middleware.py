...
class TestFlatpageMiddleware(TestDataMixin):
    @pytest.fixture(autouse=True)
    ...
    async def test_fallback_authenticated_flatpage(self):
        "A flatpage served by the middleware can require authentication"
        response = await self.client.get("/sekrit/")
        assert response.status_code == 302
        assert response.url == "/accounts/login/?next=/sekrit/"
        user = await sync_to_async(User.objects.create_user)(
            "testuser", "test@example.com", "s3krit"
        )
        await self.client.aforce_login(user)
    ...
    async def test_fallback_flatpage_special_chars(self):
        """
        ...
        response = await self.client.get("/some.very_special~chars-here/")
        assert b"<p>Isn't it special!</p>" in response.content


@pytest.mark.django_db(transaction=True)
class TestFlatpageMiddlewareAppendSlash(TestDataMixin):
    @pytest.fixture(autouse=True)
    def set_settings(self):
        old_installed_apps = settings.INSTALLED_APPS
        old_middleware = settings.MIDDLEWARE
    ...
    async def test_redirect_fallback_flatpage_special_chars(self):
        """
        A flatpage with special chars in the URL can be served by the fallback
        ...
        response = await self.client.get("/some.very_special~chars-here")
        assert response.status_code == 301
        assert response.url == "/some.very_special~chars-here/"

    async def test_redirect_fallback_flatpage_root(self):
        ...
        assert b"<p>Root</p>" in response.content
